...
public class RepositoryContext {
    ...
    private String getStatement(boolean exclude) {
        ...
        return "select new com.edusyspro.api.dto.custom.EnrolledStudent(e.id, e.student.id, e.student.personalInfo, e.academicYear," +
                "e.enrollmentDate, e.classe.id, e.classe.name, e.classe.category, e.classe.grade.section, e.classe.grade.gradingScaleMax, e.classe.grade.gradingPassThreshold, e.isArchived," +
                "e.classe.monthCost, e.student.dadName, e.student.momName, e.academicYear.school.name) from EnrollmentEntity e where " +
