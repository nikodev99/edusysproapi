...
public interface PaymentRepository extends JpaRepository<Payments, UUID> {
    @Query("""
        ...
        ind.image, i.id, i.invoiceDate, i.dueDate, i.invoiceNumber, i.discount, i.taxAmount, i.totalAmount, p.amountPaid,
        p.currency, p.transactionId, p.voucherNumber, p.paymentDate, p.paymentGateway, p.paymentMethod, p.status, p.notes, pro.id, pro.firstName, pro.lastName,
        pro.reference, pro.image, c.id, c.firstName, c.lastName, c.reference, c.image) FROM Payments p JOIN p.student e
        JOIN p.invoice i JOIN e.student s JOIN s.personalInfo ind JOIN p.processedBy pro JOIN p.createdBy c WHERE s.guardian.id = ?1
